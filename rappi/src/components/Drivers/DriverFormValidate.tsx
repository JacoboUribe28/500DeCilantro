...
interface DriverFormValidateProps {
...
interface DriverFormValidateState {}

class DriverFormValidate extends Component<DriverFormValidateProps, DriverFormValidateState> {
  ...
  handleSubmit = (values: Driver, actions: FormikHelpers<Driver>) => {
  ...
  render() {
    const initialValues: Driver = this.props.driver || {
      id: "",
      name: "",
      license_number: "",
      email: "",
      ...
      >
        {({ isSubmitting, errors, touched }) => (
          <Form className="space-y-6 max-w-md mx-auto bg-white p-6 rounded-lg shadow-lg">
            ...
            >
              {isSubmitting ? "Procesando..." : `${mode} Conductor`}
            </button>
          </Form>
        )}
      </Formik>
